...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'bank_transfer_receipts_transfer_number_digits'
  ) then
    alter table bank_transfer_receipts
      add constraint bank_transfer_receipts_transfer_number_digits
      check (transfer_number ~ '^[0-9]{4,30}$')
      not valid;
  end if;
end $$;
...
where platform = 'android'
  and version_id <> 'android-0.1.7-8';
